...
class NDTMCL3DNode {
	...
	ros::Publisher mcl_particle_pub; ///< The output of MCL is published with this!
	...
	NDTMCL3DNode(ros::NodeHandle param_nh) {
	    ...
	    mcl_particle_pub = nh_.advertise<geometry_msgs::PoseArray>("particlecloud",2);
		...
		const nav_msgs::Odometry::ConstPtr& odo_in)
	{
		...
		ndtmcl->initializeFilter(tr[0], tr[1],tr[2],rot[0],rot[1],rot[2],1.5, 1.5, 0.1, 2.0*M_PI/180.0, 2.0*M_PI/180.0 ,12.0*M_PI/180.0, 250);
	    ...
	    cloud_msg.poses.resize(ndtmcl->pf.pcloud.size());
	    for(unsigned int i=0;i<ndtmcl->pf.pcloud.size();i++){		
	      Eigen::Vector3d rot = ndtmcl->pf.pcloud[i].T.rotation().eulerAngles(0,1,2);
	      Eigen::Vector3d tr = ndtmcl->pf.pcloud[i].T.translation();
	      tf::poseTFToMsg(tf::Pose(tf::createQuaternionFromYaw(rot[2]),
		tf::Vector3(tr[0],
			    tr[1], 0)),
		cloud_msg.poses[i]);
	    }
	    mcl_particle_pub.publish(cloud_msg);
	    //publish pose
